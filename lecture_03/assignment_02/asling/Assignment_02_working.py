...
# wheel2 = wheel.transformed(Translation.from_vector([length+(2*radiusb),0,0]))
# body.transform(Translation.from_vector([length+radiusb, 0, 0]))
...
# shoulder.transform(Translation.from_vector([-(radiuss-length),0,line.length]))
# shoulder2 = shoulder.transformed(Translation.from_vector([(2*radiuss)+(2*radiusb),0,0]))
...
# P = Plane([arm.height,0,0], [0,0,1])
# arm2 = arm.transformed(Reflection.from_plane(P))
arm2 = arm.transformed(Translation.from_vector([-arm.height, 0, 0]))
# need to transform arm to right angle
