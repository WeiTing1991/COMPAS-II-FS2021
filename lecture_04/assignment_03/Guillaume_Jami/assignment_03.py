...
def calculate_ik(robot, frame):
    ...
    start_configuration = Configuration.from_revolute_values([-1.100, -1.153, -2.174, 0.185, 0.667, 0.])
...
def store_configurations(configurations):
    ...
    PATH = os.path.abspath(os.path.join(HERE, "configurations_GJ.json"))

    # list to dictionnaries
    configurations_list = []
    for configuration in configurations:
        configurations_list.append(configuration.data)

    write_data_to_json(configurations_list, PATH)
